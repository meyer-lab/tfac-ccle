...
timee = np.linspace(0.0, 94.0, 189)
g1g2 = ["G1", "S/G2"]
...
def makeFigure():
    ...
    times = pd.DataFrame(tFac.factors[2], columns=[f"Cmp. {i}" for i in np.arange(1, tFac.rank + 1)], index=timee)
    ...
    ax[4].plot(timee, times.to_numpy())
    ax[4].set_xlabel("Time")
    ax[4].set_ylabel("Component Weight")
    ax[4].legend(["Cmp. 1", "Cmp. 2", "Cmp. 3"])
