...
def makeFigure():
    ...
    proteinBoxPlot(ax[1], GCPHistones[:, 0], 1, histones, 'Histones')
    proteinBoxPlot(ax[2], L1000GeneExp[:, 0], 1, geneExpression, 'Genes')
    proteinBoxPlot(ax[3], RPPAproteins[:, 0], 1, Rproteins, 'Protein Factors')
    subplotLabel(ax)
    return f
