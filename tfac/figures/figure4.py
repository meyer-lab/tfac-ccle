...
def makeFigure():
    ...
    proteinBoxPlot(ax[1], GCPHistones[:, 3], 4, histones, 'Histones')
    proteinBoxPlot(ax[2], L1000GeneExp[:, 3], 4, geneExpression, 'Genes')
    proteinBoxPlot(ax[3], RPPAproteins[:, 3], 4, Rproteins, 'Protein Factors')
    subplotLabel(ax)
    return f
