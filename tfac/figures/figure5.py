...
def makeFigure():
    ...
    tensor, ligand, ecm, measurements = import_LINCS_MEMA("hmec122l_ssc_Level4.tsv.xz")
    fac = parafac(tensor, 5, n_iter_max=2000, linesearch=True, tol=1e-9)
    fac = cp_flip_sign(fac, 2)
    fac = cp_normalize(fac)

    facZero = pd.DataFrame(fac.factors[0], columns=[f"Cmp. {i}" for i in np.arange(1, fac.rank + 1)], index=ligand)
    facOne = pd.DataFrame(fac.factors[1], columns=[f"Cmp. {i}" for i in np.arange(1, fac.rank + 1)], index=ecm)
    facTwo = pd.DataFrame(fac.factors[2], columns=[f"Cmp. {i}" for i in np.arange(1, fac.rank + 1)], index=measurements)
