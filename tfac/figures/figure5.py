...
def makeFigure():
    ...
    proteinBoxPlot(ax[1], GCPHistones[:, 4], 5, histones, 'Histones')
    proteinBoxPlot(ax[2], L1000GeneExp[:, 4], 5, geneExpression, 'Genes')
    proteinBoxPlot(ax[3], RPPAproteins[:, 4], 5, Rproteins, 'Protein Factors')
    subplotLabel(ax)
    return f
