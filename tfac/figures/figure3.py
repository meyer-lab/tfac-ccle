...
def makeFigure():
    ...
    proteinBoxPlot(ax[1], LINCSproteins[:, 2], 3, proteins)
    proteinBoxPlot(ax[2], GCPHistones[:, 2], 3, histones)
    proteinBoxPlot(ax[3], L1000GeneExp[:, 2], 3, geneExpression)
    proteinBoxPlot(ax[4], RPPAproteins[:, 2], 3, Rproteins)
    subplotLabel(ax)
    return f
